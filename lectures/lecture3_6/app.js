...
class App{
    ...
    updateGamepadState(){
        const session = this.xr.renderer.xr.getSession();
        ...
        if (inputSource && inputSource.gamepad && this.gamepadIndices && this.ui && this.buttonStates){
            ...
            try{
                object.entries(this.buttonStates ).forEach(([key,value]) =>{
                    ...
                    if(key.indexOf('touchapd')!=-1||key.indexOF('thumbstick')!=-1){
                        ...
                        this.buttonStates[key].xAxis = gamepad.axes[yAxisIndex].toFixed(2);
                ...
                console.log("An error occurred settin the UI")
            }
        }
    }
    ...
    setupXR(){
        ...
        function onConnected( event ){
            ...
            fetchProfile(event.data,DEFAULT_PROFILES_PATH,DEFAULT_PROFILE).then(({profile,assetPath})=>{
                ...
                info.targetRayMode = event.target.targetRayMode;

                object.entries(profile.layouts).forEach(([key,layout]) =>{
                    const components = {};
                    Object.values(layout.components).forEach(([key,layout])=>{
                        components[components.rootNodeName] = components.gamepadIndices;
                    });
                    info[key] = components;
                });
